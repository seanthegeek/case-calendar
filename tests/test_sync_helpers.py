...
class TestIsFetchable:
    def test_paperless_placeholder(self):
        assert not _is_fetchable({
            "is_available": False, "is_sealed": None,
            "filepath_local": None, "filepath_ia": "", "plain_text": "",
        })

    def test_sealed(self):
        assert not _is_fetchable({
            "is_available": False, "is_sealed": True,
            "filepath_local": None, "filepath_ia": "", "plain_text": "",
        })
    ...
    def test_available_with_filepath_local(self):
        assert _is_fetchable({
            "is_available": True, "filepath_local": "recap/foo.pdf",
            "filepath_ia": "", "plain_text": "",
        })

    def test_available_with_filepath_ia(self):
        assert _is_fetchable({
            "is_available": True, "filepath_local": None,
...
class TestNeedsPdf:
    ...
    def test_entered_paren_form_also_stripped(self):
        assert _needs_pdf(_entry(
            "Notice of Hearing (Entered: 03/13/2026 11:30 AM)"
        ))
...
class TestExtractDocketRefs:
    def test_grants_numbered_motion(self):
        refs = _extract_docket_refs(_entry(
            "ORDER granting 65 Motion for Hearing as to Ashtor."
        ))
        assert refs == [65]
    ...
    def test_grants_in_part(self):
        refs = _extract_docket_refs(_entry(
            "ORDER granting in part 100 Motion to Compel."
        ))
        assert refs == [100]

    def test_bracketed_form(self):
        refs = _extract_docket_refs(_entry(
            "Response to [42] Motion to Dismiss filed by Plaintiff."
        ))
        assert refs == [42]

    def test_dedupes(self):
        refs = _extract_docket_refs(_entry(
            "ORDER granting 65 Motion for Hearing. See 65 for full text."
        ))
        assert refs == [65]
    ...
    def test_entered_footer_date_not_picked_up(self):
        ...
        refs = _extract_docket_refs(_entry(
            "ORDER granting 65 Motion. (Entered: 12/26/2025)"
        ))
        assert refs == [65]
...
class TestLocalToUtc:
    def test_naive_local_time_with_eastern(self):
        ...
        assert _local_to_utc("2026-01-07", "15:00", "America/New_York") == \
            "2026-01-07T20:00:00+00:00"

    def test_eastern_summer_offset(self):
        # 3pm EDT in April = 19:00 UTC
        assert _local_to_utc("2026-04-14", "15:00", "America/New_York") == \
            "2026-04-14T19:00:00+00:00"

    def test_pacific_time(self):
        assert _local_to_utc("2026-04-14", "10:00", "America/Los_Angeles") == \
...
class TestFingerprint:
    def test_stable_for_identical_input(self):
        e = {"id": 1, "description": "x", "short_description": "",
             "date_filed": "2026-01-01", "recap_documents": []}
        assert fingerprint_entry(e) == fingerprint_entry(dict(e))

    def test_changes_when_description_changes(self):
        a = {"description": "x", "short_description": "", "date_filed": "d",
             "recap_documents": []}
        b = dict(a, description="y")
        assert fingerprint_entry(a) != fingerprint_entry(b)

    def test_changes_when_pdf_becomes_available(self):
        before = {"description": "Notice of Hearing", "short_description": "",
                  "date_filed": "d",
                  "recap_documents": [{"description": "Notice", "is_available": False,
                                       "is_sealed": None, "plain_text": ""}]}
        after = dict(before)
        after["recap_documents"] = [{"description": "Notice", "is_available": True,
                                     "is_sealed": None, "plain_text": "the body"}]
    ...
    def test_changes_when_text_becomes_available(self):
        before = {"description": "x", "short_description": "", "date_filed": "d",
                  "recap_documents": [{"description": "doc", "is_available": True,
                                       "is_sealed": None, "plain_text": ""}]}
        after = dict(before)
