...
> These prompts are mirrored from [`case_calendar/llm.py`](https://github.com/seanthegeek/case-calendar/blob/main/case_calendar/llm.py) as of **v0.13.1**. The verbatim prompt blocks, their source links, and this version stamp are generated by [`scripts/sync_llm_prompts_doc.py`](https://github.com/seanthegeek/case-calendar/blob/main/scripts/sync_llm_prompts_doc.py) (CI fails if this page drifts from the source), so edit `llm.py` and re-run the script rather than editing this page by hand. `llm.py` is canonical — if a prompt here ever disagrees with the source, trust the source and [open an issue](https://github.com/seanthegeek/case-calendar/issues/new/choose).
